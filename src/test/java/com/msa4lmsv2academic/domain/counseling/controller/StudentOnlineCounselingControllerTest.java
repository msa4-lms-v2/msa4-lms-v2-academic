...
class StudentOnlineCounselingControllerTest {

    private final StudentOnlineCounselingService studentOnlineCounselingService =
            mock(StudentOnlineCounselingService.class);
    private final StudentOnlineCounselingController studentOnlineCounselingController =
            new StudentOnlineCounselingController(studentOnlineCounselingService);
    ...
    void createOnlineReturnsCreated() {
        ...
        when(studentOnlineCounselingService.createOnline(request, student)).thenReturn(record);

        ResponseEntity<GlobalRes<CounselingRecordResponseDTO>> response =
                studentOnlineCounselingController.createOnline(request, student);
    ...
    void searchMyRecordsReturnsGlobalPagedResponse() {
        ...
        when(studentOnlineCounselingService.searchMyRecords(request, student)).thenReturn(page);

        ResponseEntity<?> response = studentOnlineCounselingController.searchMyRecords(request, student);
    ...
    void getMyRecordReturnsProfessorResponse() {
        CounselingRecordResponseDTO record = response();
        when(studentOnlineCounselingService.getMyRecord(11L, student)).thenReturn(record);

        ResponseEntity<GlobalRes<CounselingRecordResponseDTO>> response =
                studentOnlineCounselingController.getMyRecord(11L, student);

        assertThat(response.getStatusCode().is2xxSuccessful()).isTrue();
        assertThat(response.getBody()).isNotNull();
        assertThat(response.getBody().data().professorResponse())
                .isEqualTo("전공필수 6학점이 더 필요합니다.");
    }
