package com.msa4lmsv2academic.global.scheduling;
 ...
 * 배포 pod에서 실행되지 않는 현상이 OutboxWorker에서 확인된 바 있어(2026-09-04, 커밋 2fb3a70/a0cea71),
 * cron 표현식이 필요한 스케줄러는 Spring 스케줄링 인프라 대신 이 유틸로 직접 다음 실행 시각을 계산해
 * ScheduledExecutorService에 재귀 예약한다.
 */
public final class CronScheduling {
    ...
    // Spring @Scheduled(cron=...)와 동일하게 "-"는 비활성화로 취급한다(테스트의 idempotency-cleanup.cron=- 설정과 호환).
