...
public class LeaveRequestController {
    ...
    @Operation(operationId = "createLeaveRequest", summary = "휴·복학 신청",
            description = "STUDENT 본인만 multipart request(JSON)와 files 배열을 제출합니다. 일반휴학은 사유 필수·증빙 선택, "
                    ...
                    + "일반휴학은 현재 학기의 다음 학기에만 적용되고 복학 예정은 그 바로 다음 학기로 고정됩니다. 휴학 중인 학생도 다음 적용 학기의 활성 접수 기간에 일반휴학을 다시 신청할 수 있습니다. "
    ...
    public ResponseEntity<GlobalResponseDTO<LeaveRequestResponseDTO>> create(
            ...
            @Parameter(description = "1~100자의 공백 없는 요청별 키. 같은 사용자·경로·본문·파일 목록의 완료 응답은 24시간 재생. 재생 시 업무·감사 재실행 없음",
        ...
        if (files != null) submittedFiles.addAll(files);
        if (legacyFile != null && !legacyFile.isEmpty()) submittedFiles.add(legacyFile);
        return ResponseEntity.ok(GlobalResponseDTO.success(applicationService.create(request, submittedFiles, key, actor,
    ...
    public ResponseEntity<byte[]> download(
            ...
            @Parameter(hidden = true) @AuthenticationPrincipal CurrentUser actor) {
        var download = applicationService.download(id, fileId, actor);
    ...
    public ResponseEntity<byte[]> downloadFirst(
            @Parameter(description = "신청 ID", example = "1") @Positive @PathVariable Long id,
            @Parameter(hidden = true) @AuthenticationPrincipal CurrentUser actor) {
        return downloadResponse(applicationService.downloadFirst(id, actor));
    }
