...
public class ProfessorLectureController {
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "401", description = "인증 실패",
                    content = @Content(schema = @Schema(implementation = GlobalRes.class))),
            @ApiResponse(responseCode = "403", description = "교수 권한이 아님",
                    content = @Content(schema = @Schema(implementation = GlobalRes.class))),
            @ApiResponse(responseCode = "404", description = "교수 정보 없음",
                    content = @Content(schema = @Schema(implementation = GlobalRes.class)))
    })
    @GetMapping
    @PreAuthorize("hasRole('PROFESSOR')")
    public ResponseEntity<GlobalRes<PageRes<ProfessorLectureResponseDTO>>> getMyLectures(
