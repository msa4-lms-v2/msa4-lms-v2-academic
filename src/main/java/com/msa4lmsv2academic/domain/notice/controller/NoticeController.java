...
public class NoticeController {
    ...
    @Operation(
            ...
            description = "STUDENT와 PROFESSOR는 활성 상태인 ALL 또는 본인 역할 대상 공지만 조회합니다. "
                    ...
                    + "ADMIN은 제목·본문·작성자·분류·대상·상태·작성 기간으로 검색할 수 있습니다.",
    ...
    @Operation(
            ...
            description = "ADMIN만 공지사항을 등록할 수 있습니다. 공지는 활성 상태로 생성되고 작성자 ID는 "
                    + "Gateway 인증 사용자로 기록합니다. application/json과 multipart/form-data(첨부파일 files)를 모두 지원합니다.",
            security = @SecurityRequirement(name = "bearerAuth")
    )
    ...
    @Operation(
            summary = "첨부파일 포함 공지사항 등록",
            description = "ADMIN만 multipart/form-data의 request(JSON)와 files(선택)를 함께 전송해 공지를 등록합니다. "
                    + "첨부파일은 PDF, JPEG/PNG/GIF/WebP, HWP/HWPX만 최대 5개, 파일당 10MB·전체 20MB까지 허용합니다.",
            security = @SecurityRequirement(name = "bearerAuth")
    )
    @ApiResponse(responseCode = "201", description = "등록 성공")
    @CustomApiResponse({
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<GlobalResponseDTO<NoticeDetailResponseDTO>> createNoticeWithAttachments(
            @Parameter(description = "공지사항 등록 JSON") @Valid @RequestPart("request") NoticeCreateRequestDTO request,
        ...
        NoticeDetailResponseDTO response = noticeService.createNotice(
                request, files, currentUser, requestId, httpServletRequest.getRemoteAddr()
        );
        return ResponseEntity.status(HttpStatus.CREATED).body(GlobalResponseDTO.success(response));
    }
    ...
    @Operation(
            ...
            description = "ADMIN만 제목, 본문, 대상 역할, 활성 상태를 부분 수정합니다. 비활성 공지도 수정·복구할 수 "
                    ...
                    + "application/json과 multipart/form-data(첨부파일 추가)를 모두 지원합니다.",
            security = @SecurityRequirement(name = "bearerAuth")
    )
    ...
    @Operation(
            summary = "첨부파일 포함 공지사항 수정",
            description = "ADMIN만 multipart/form-data의 request(JSON)와 files(선택)를 함께 전송해 공지 정보를 수정하고 첨부파일을 추가합니다. "
                    + "기존 첨부파일은 유지됩니다.",
            security = @SecurityRequirement(name = "bearerAuth")
    )
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<GlobalResponseDTO<NoticeDetailResponseDTO>> updateNoticeWithAttachments(
            @PathVariable @Positive(message = "noticeId는 양수여야 합니다.") Long noticeId,
            @Parameter(description = "공지사항 수정 JSON") @Valid @RequestPart("request") NoticeUpdateRequestDTO request,
            @Parameter(description = "추가 첨부파일 목록", required = false) @RequestPart(value = "files", required = false) List<MultipartFile> files,
            @Parameter(hidden = true) @AuthenticationPrincipal CurrentUser currentUser,
            @RequestHeader(value = "X-Request-Id", required = false) String requestId,
            @Parameter(hidden = true) HttpServletRequest httpServletRequest
    ) {
        return ResponseEntity.ok(GlobalResponseDTO.success(noticeService.updateNotice(
                noticeId, request, files, currentUser, requestId, httpServletRequest.getRemoteAddr()
        )));
    }
