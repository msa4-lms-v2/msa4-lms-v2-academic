...
@Component
public class WithdrawalFileValidator {
    ...
    public void validateRequired(MultipartFile file) {
        if (file == null || file.isEmpty()) {
            throw new InvalidFileException("자퇴 증빙 파일은 필수입니다.");
        }
        if (file.getSize() > MAX_FILE_SIZE) {
            throw new FileSizeExceededException("자퇴 증빙 파일은 10MB 이하여야 합니다.");
        }
        String originalName = file.getOriginalFilename();
        if (originalName == null || originalName.isBlank()) {
            throw new InvalidFileException("파일명이 없는 자퇴 증빙은 업로드할 수 없습니다.");
        }
        ...
        throw new InvalidFileException(
                "자퇴 증빙은 PDF, 이미지(JPEG, PNG, GIF, WebP), HWP 또는 HWPX 형식만 허용됩니다."
        );
    }
    ...
    private String detectedContentType(MultipartFile file) {
        try (TikaInputStream inputStream = TikaInputStream.get(file.getInputStream())) {
            ...
            throw new InvalidFileException("자퇴 증빙 파일을 검사할 수 없습니다.", exception);
        }
    }
