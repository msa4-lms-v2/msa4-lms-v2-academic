...
import com.msa4lmsv2academic.global.error.InvalidDismissalRequestException;
...
public class DismissalIdempotencyService {
    ...
    public void validateKey(String key) {
        if (key == null || key.isBlank() || key.length() > 100 || key.chars().anyMatch(Character::isWhitespace)) {
            throw new InvalidDismissalRequestException("1~100자의 공백 없는 Idempotency-Key가 필요합니다.");
        }
    }
    ...
    public <T> Optional<T> replay(String key, Long userId, String endpoint, String hash,
        ...
        if (!saved.matches(userId, endpoint, hash) || saved.getStatus() != IdempotencyStatus.COMPLETED) {
            throw new DismissalConflictException("다른 요청에 사용되었거나 처리 중인 멱등성 키입니다.");
        }
        ...
        try {
            ...
            for (Throwable cause = exception; cause != null; cause = cause.getCause()) {
                if (cause instanceof java.sql.SQLException sql && sql.getErrorCode() == 1062
                    ...
                    throw new DismissalConflictException("동일한 멱등성 키가 동시에 사용되었습니다.");
                }
            }
            throw exception;
        }
