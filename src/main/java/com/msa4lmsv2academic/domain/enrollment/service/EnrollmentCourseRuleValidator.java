...
/**
 * 신규 수강신청 저장 전에 최대학점·재수강 성적 기준·동일 교과목 수강 횟수만 검증한다.
...
public class EnrollmentCourseRuleValidator {
    ...
    private final RetakeEligibilityValidator retakeEligibilityValidator;
    ...
    public void validate(Long studentId, Lecture lecture) {
        ...
        long attemptCount = retakeEligibilityValidator.validateAndCount(studentId, lecture.getCourse());
        if (attemptCount >= 2) {
            throw new EnrollmentCourseAttemptLimitNotAllowedException(
                    EnrollmentCourseAttemptLimitRejectionReason.COURSE_ATTEMPT_LIMIT_EXCEEDED
            );
        }
    }
}
